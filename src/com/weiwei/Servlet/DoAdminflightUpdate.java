...
/**
 * Servlet implementation class DoAdminflightUpdate
 */
@WebServlet("/DoAdminflightUpdate")
public class DoAdminflightUpdate extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DoAdminflightUpdate() {
        super();
	...
	flight fb=new flight(fid,fname,fchufa,fdaoda,fchufajichang,fdaodajichang,d,fchufatime,fdaodatime,ffy,fby,fey,ffp,fbp,fep);
		
		int jg = UsersTableDao.AdminflightUpdate(fb);
		
		System.out.println(fname+"--"+fchufa+"--"+fdaoda+"--"+fchufajichang+"--"+fdaodajichang+"--"+d+"--"+fchufatime+"--"+fdaodatime+"--"+ffy+"--"+fby+"--"+fey+"--"+ffp+"--"+fbp+"--"+fep);
		
		System.out.println(jg);
		if(jg>0){
			response.sendRedirect("./DoAdminSelctFlight");
		}else{
			response.sendRedirect("Admin/AdminflightUpdate.jsp");
		}
